...
public class ProdavnicaMenadzer extends Thread {
    public void run(){
        ...
        while(true){
            ...
            if (message instanceof ObjectMessage){
                try {
                    if (((ObjectMessage) message).getObject() instanceof PorukaZaRezervaciju){
                        ...
                        Query query1 = em.createQuery("select stanje from Stanje as stanje where stanje.prodavnica = :naziv and stanje.artikal = :sifra", Stanje.class);
                        List<Stanje> stanja = query1.setParameter("naziv", prodavnica).setParameter("sifra", rezervacija.getSifartikla()).getResultList();
                        for (Stanje stanje : stanja){
                            stanja.get(0).setKolicina(stanja.get(0).getKolicina() - rezervacija.getKolicina());
                        }
                        em.getTransaction().commit();
            ...
            if (message instanceof ObjectMessage){
                try {
                    if(((ObjectMessage) message).getObject() instanceof PorukaZaArtikal){
                        ObjectMessage objectMessage = (ObjectMessage) message;
                        PorukaZaArtikal porukaZaArtikal = (PorukaZaArtikal) objectMessage.getObject();
                        Query query = em.createQuery("select stanje from Stanje as stanje where stanje.prodavnica.naziv = :naziv and stanje.artikal.naziv = :naziv2", Stanje.class);
                        List<Stanje> stanja = query.setParameter("naziv", Main.StoreName).setParameter("naziv2", porukaZaArtikal.getNazivArtikla()).getResultList();
                        em.getTransaction().begin();
                        if (!stanja.isEmpty()){
                            stanja.get(0).setKolicina(stanja.get(0).getKolicina() + porukaZaArtikal.getKolicina());
                        }
                        else{
                            Query q = em.createQuery("select artikal from Artikal as artikal where artikal.naziv = :naziv", Artikal.class);
                            List<Artikal> artikli = q.setParameter("naziv", porukaZaArtikal.getNazivArtikla()).getResultList();
                            Stanje stanje = new Stanje();
                            stanje.setArtikal(artikli.get(0));
                            Prodavnica prodavnica = em.find(Prodavnica.class, Main.StoreName);
                            stanje.setProdavnica(prodavnica);
                            StanjePK stanjePK = new StanjePK(prodavnica.getSifra(), artikli.get(0).getSifra());
                            stanje.setStanjePK(stanjePK);
                            stanje.setKolicina(porukaZaArtikal.getKolicina()); 
                            em.persist(stanje);
                        }
                        em.getTransaction().commit();
                        System.out.println("Primljen artikal " + porukaZaArtikal.getNazivArtikla() + " u kolicini " + porukaZaArtikal.getKolicina());
                    }
                } catch (JMSException ex) {
